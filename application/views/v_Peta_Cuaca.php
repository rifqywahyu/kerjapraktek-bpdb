...
<body id="layouts-horizontal">
                  ...
                  <div class="row">
                    <div class="col s12 m12 l12">
                      <div class="card-panel" style="padding-top:1px;">
                        <h4 class="header2" style="text-align:center">Peta Cuaca
                          <?php if($range != ' - '){
                              echo $range;
                          }else{
                            echo "Hari Ini";
                    ...
                    <div class="row">
                      <div class="col s12 m12 l12">
                        <form class="col s12" id="formValidate" action="<?php echo base_url()?>Ms_Peta_Cuaca" method="get">
                        <div class="card-panel" style="padding-top:1px;">
                          <h4 class="header2">Filter Cuaca</h4>
           ...
           var map = new google.maps.Map(mapElement, mapOptions);
             ...
             var locations = [
               <?php
                foreach ($data as $row) { ?>
                 ['<div id="iw-container">' +
                    '<div class="iw-title">'+'<?php echo strtoupper($row->nama) ?>'+'<?php echo strtoupper($row->atensi) ?>'+'</div>' +
                    '<div class="iw-title">'+' ID INFORMASI '+'<?php echo strtoupper($row->idtblatensi) ?>'+'</div>' +
                    '<div class="iw-content">' +

                      '<div class="iw-subTitle">Tanggal & Waktu Atensi</div>' +
                      '<p><?php echo strtoupper($row->create_time)?></p>' +
                
                      '<div class="iw-subTitle">Icon</div>' +
                      <?php $data_detail = $this->db->get_where("tblcuaca", array("id" => $row->id))->result();?>
                      <?php foreach ($data_detail as $rows) { ?> 
                       <?php if ($rows->type == '1') { ?>
                          '<img src="<?php echo base_url().'uploads/cuaca/'.$rows->icon?>" data-nama="<?php echo strtoupper($row->nama) ?> 
                          <?php echo strtoupper($row->nama) ?>" class="img" height="120" width="100%"><br />' <?php
                        <?php } ?>
                      <?php } ?>
                    '</div>' +
                    '<div class="iw-bottom-gradient"></div>' +
                  '</div>',
                 <?php echo $row->latitude?>,<?php echo $row->longitude?>,'<?php echo base_url().'uploads/cuaca/'.$row->icon?>'],
               <?php } ?>
             ];
  ...
  <script>
    $(document).on('click', '.img,, function (e) {
      ...
      if (myClass == 'img') {
        $("#title_foto").html("Icon "+nama);
        $("#data_icon").attr("src", icon);
        $('#modal5').openModal();
      }else{
  ...
  <!-- <div id="modal_video" class="modal bottom-sheet" style="max-height:75%">
    <div class="modal-content">
      <div class="row">
        <div class="col s12 m8 l8">
          <h4 id="title_video">Modal Header </h4>
        </div>
        <div class="col s12 m4 l4">
          <a class="right btn waves-effect waves-light blue modal-close" type="submit" style="margin-left:5px;">Tutup <i class="mdi-content-clear right"></i></a>
        </div>
      ...
      <ul class="collection">
        <li class="collection-item avatar" style="text-align:center">
          <video width="100%" height="100%" id="vid" controls>
            <source id="data_video" src="" type="video/mp4">

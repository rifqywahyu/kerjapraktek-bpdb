...
{
  function data_cuaca($flag = '' , $from , $to){
     $sql="SELECT a.*, b.nama, b.icon FROM tblatensi a 
          LEFT JOIN tblcuaca b ON a.`idcuaca`=b.`id`";
    if ($flag !='') {
  ...
  function data_penduduk($kecamatan, $kelurahan){
    ...
    $sql = "SELECT e.nama AS nama_kec,
            e.`id_kec`, d.`id_kel`, b.`idtbldusun`,
            d.nama AS nama_kel,
        ...
        $sql .= " GROUP BY b.idtbldusun ORDER BY id_kec, id_kel";

        // die($sql);
      return $this->db->query($sql);
  }

  function sum_ternak($idtbldusun, $idtblternak){
    $sql = "SELECT IFNULL(SUM(a.jml_ternak), 0) AS jml, c.nama FROM tblkk_detail a
            LEFT JOIN tblkk b ON b.`idtblkk` = a.idtblkk
            LEFT JOIN tblternak c ON c.`idtblternak` = a.idtblternak
            WHERE a.idtblternak IS NOT NULL
            AND b.`idtbldusun` = '$idtbldusun' AND c.idtblternak = '$idtblternak'
            ;" ;
    return $this->db->query($sql);
  }
  ...
  function data_kejadian_live( $flag = '', $from = '', $to = ''){
    ...
    $sql = "SELECT a.*, b.nama AS nama_kelurahan, c.nama AS nama_bencana, c.icon, c.id, d.picture_name, e.nama as nama_dusun FROM tbllaporan a
            LEFT JOIN tblkelurahan b ON a.kelurahan = b.`id_kel`
            LEFT JOIN tblbencana c ON c.`id` = a.bencana
            LEFT JOIN tbllaporan_galery d ON d.idtbllaporan = a.idtbllaporan
            LEFT JOIN tbldusun e ON e.`idtbldusun` = a.`dusun`
            WHERE a.tgl_kejadian between '$dari 00:00:01' AND '$hingga 23:59:59' ";
    if ($flag != '') {
      $sql .= " AND a.flag = '$flag' ";
    }
    // $sql .= " Limit 0,10";
  ...
  function sql_data_live(){
		$now = date('Y-m-d');
		$sql = "SELECT * FROM (SELECT DISTINCT b.nama, a.latitude, a.longitude, a.create_time FROM tbltracking a
            LEFT JOIN tbluser b ON b.idtbluser= a.idtbluser
            WHERE a.create_time BETWEEN '$now 00:00:01' AND '$now 23:59:59' ORDER BY a.create_time DESC) a GROUP BY nama ORDER BY a.create_time DESC;
          ";
		// die($sql);
		return $this->db->query($sql);
	}
...
//    die($bencana.'__'.$status.'___'.$dari.'___'.$hingga.'___'.$kecamatan.'___'.$kelurahan);
    ...
    $sql = "SELECT
            a.`idtbllaporan`,
            a.`tgl_kejadian` AS tanggal_kejadian,
            a.create_time as waktu_buat,
            d.`nama` AS nama_bencana,
            a.`rt`,
            a.`rw`,
            b.`nama` AS nama_desa,
            c.`nama` AS nama_kecamatan,
            e. nama as nama_dusun,
            a.`latitude`,
            a.`longitude`,
            a.dampak,
            a.rangkuman_korban
            ...
            LEFT JOIN tbldusun e ON e.idtbldusun = a.dusun
            LEFT JOIN tblbencana d ON d.`id` = a.`bencana` WHERE a.tgl_kejadian BETWEEN '$dari 00:00:01' AND '$hingga 23:59:59' ";
  ...
  function detail_kk($idtblkk){
    $sql = "SELECT a.nama, TIMESTAMPDIFF(YEAR,a.tanggal_lahir, CURDATE()) AS usia , b.nama AS nama_ternak, a.type, a.status, a.jml_ternak FROM tblkk_detail a
            ...
            WHERE idtblkk = '$idtblkk' AND a.type = '1' ";
    // echo $sql
    return $this->db->query($sql);
  }
  ...
  function sub_header_bencana($bencana, $status, $dari, $hingga, $kecamatan, $kelurahan){
    ...
    $sql = "SELECT c.id_kec, c.nama AS nama_kecamatan, b.`id_kel`, b.nama AS nama_kelurahan, a.`idtbllaporan`,total, jml_kec FROM tbllaporan a
            ...
            if ($bencana != 'bencana_all' && $bencana != '' ) {
              $sql .= " WHERE `bencana` = '$bencana' ";
              $sql .= "AND a.tgl_kejadian between '$dari 00:00:01' AND '$hingga 23:59:59'";
            }else{
              $sql .= "WHERE a.tgl_kejadian between '$dari 00:00:01' AND '$hingga 23:59:59'";
            }

...
class Ms_cuaca extends MY_Controller {
    ...
    $data['akses'] = $this->akses_array;
        $this->load->view('v_ms_cuaca', $data);
    }

    function tambah(){
      $this->load->view("v_ms_cuaca_a");
    }
    ...
    function proses_tambah(){
      ...
      if (isset($_FILES['foto'])) {
        if(! $_FILES['foto']['size'] == '0' ){
          ...
          $this->db->insert("tblcuaca", array(
            "nama" => $nama, "ket" => $ket, "create_time" => date("Y-m-d H:i:s")
          ));
    ...
    function upload_image($files, $id){
        ...
        if (! $this->upload->do_upload('foto'))
        {
          ...
          $this->db->where("id", $id);
          $this->db->update("tblcuaca", array("icon" => $fname));
        }
    }

    function edit($id_md5){
      $data['data'] = $this->db->get_where("tblcuaca", array("md5(id)" => $id_md5) )->row();
      // die(print_r($data));
      $this->load->view("v_ms_cuaca_e", $data);
    ...
    function get_cuaca(){
      ...
      $datax = $this->_ms->get_data($search, "tblcuaca")->result();
		  $data = array();
      ...
      foreach ($datax as $rows){
        ...
        if($this->akses_array['update'] == '1'){
          $row[4] = '<div style="margin-left: 20px;" class="btn-group"> <a href="'.base_url().'ms_cuaca/edit/'.md5($rows->id).'" class="blue btn-small btn "><i class="mdi-editor-border-color right"></i>&nbsp; Ubah</a>';
        }
        if($this->akses_array['delete'] == '1'){
          $row[4] .= '<a href="'.base_url().'ms_cuaca/delete/'.md5($rows->id).'" data-nama="'.$rows->nama.'" class="btn-small btn link_delete"><i class="right mdi-navigation-cancel"></i>Hapus</a> </div>';
        }
        $data[] = $row;
			}
    ...
    function delete($id){
      ...
      $this->db->update('tblcuaca', array("flag" => "1"));
    ...
    function delete_pict($id_md5, $name){
      ...
      $file = './uploads/cuaca/'.$name;
      if(unlink($file)){
        $this->db->where("md5(id)", $id_md5);
        $this->db->update("tblcuaca", array("icon" => ''));
      }else{
        echo "gagal hapus";
      }
       echo '1';

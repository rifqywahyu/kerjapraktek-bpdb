...
class Ms_pengungsian extends MY_Controller {
    ...
    {
        $this->load->view('v_ms_pengungsian');
    }

    function tambah(){
      $this->load->view("v_ms_pengungsian_a");
    }
    ...
    function proses_tambah(){
      ...
      if (isset($_FILES['foto'])) {
        if(! $_FILES['foto']['size'] == '0' ){
          ...
          $this->db->insert("tblpengungsian", array(
            "nama" => $nama, "deskripsi" => $deskripsi, "create_time" => date("Y-m-d H:i:s"),
            "alamat" => $alamat, "penanggung_jawab" => $penanggung_jawab, "tlpn" => $tlpn,
    ...
    function upload_image($files, $id){
        ...
        if (! $this->upload->do_upload('foto'))
        {
          ...
          $this->db->where("idtblpengungsian", $id);
          $this->db->update("tblpengungsian", array("picture_name" => $fname));
        }
    }

    function edit($id_md5){
      $data['data'] = $this->db->get_where("tblpengungsian", array("md5(idtblpengungsian)" => $id_md5) )->row();
      $this->load->view("v_ms_pengungsian_e", $data);
    ...
    function get_pengungsian(){
      ...
      foreach ($datax as $rows){
        ...
        $row[] = '<div style="margin-left: 20px;" class="btn-group">
                ...
                <a href="'.base_url().'ms_pengungsian/edit/'.md5($rows->idtblpengungsian).'" class="blue btn-small btn " style="width:3%;"> <i class="mdi-editor-border-color right"></i></a>
                <a href="'.base_url().'ms_pengungsian/delete/'.md5($rows->idtblpengungsian).'" data-nama="'.$rows->nama.'" class="btn-small btn link_delete" style="width:3%;"> <i class="right mdi-navigation-cancel"></i></a> </div>';
    ...
    function proses_edit(){
      ...
      $this->db->where("idtblpengungsian", $id);
      $this->db->update("tblpengungsian", array(
        "nama" => $nama, "deskripsi" => $deskripsi, "create_time" => date("Y-m-d H:i:s"),
        "alamat" => $alamat, "penanggung_jawab" => $penanggung_jawab, "tlpn" => $tlpn,
        "tampungreguler" => $tampungreguler, "tampungprotokol" =>$tampungprotokol, "bilik" => $bilik,
        "latitude" => $latitude, "longitude" => $longitude
    ...
    function delete_pict($id_md5, $name){
      ...
      $file = './uploads/pengungsian/'.$name;
      if(unlink($file)){
        $this->db->where("md5(idtblpengungsian)", $id_md5);
        $this->db->update("tblpengungsian", array("picture_name" => ''));
      }else{
        echo "gagal hapus";
      }
       echo '1';
